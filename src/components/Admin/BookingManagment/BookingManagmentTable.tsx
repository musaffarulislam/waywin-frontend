  ...
  return (
    <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
        ...
        <tbody>
          {bookings && bookings.length > 0 ? (
            bookings
              ...
              .map((booking, index) => {
                const bookingObject = typeof booking === "string" ? JSON.parse(booking) : booking;
                return (
                  <tr
                    key={index}
                    className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600"
                  >
                    <td className="px-6 py-6">{bookingObject?.trainerId?.authId?.username}</td>
                    <td className="px-6 py-6">{bookingObject?.userId?.username}</td>
                    <td className="px-6 py-6">{bookingObject?.service}</td>
                    <td className="px-6 py-6">{bookingObject?.mode}</td>
                    <td className="px-6 py-6">{bookingObject?.fee}</td>
                    <td className="px-6 py-6">{format(new Date(bookingObject.date), "dd MMMM yyyy")}</td>
                    <td className="px-6 py-6">{new Date(bookingObject.time).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })}</td>
                  </tr>
                );
              })
          ) : (
      ...
      <div className="flex justify-center py-4">
        ...
        <button
          disabled={!bookings || bookings.length === 0 || bookings.length <= currentPage * rowsPerPage}
          onClick={() => setCurrentPage((prev) => prev + 1)}
          className=" px-3 py-1 text-xl font-medium text-gray-900 rounded bg-gray-200 dark:bg-gray-700 dark:text-gray-300"
        >
          Next
