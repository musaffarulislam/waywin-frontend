...
import { changeUserStatus, getAllUsersInfo } from "../../../app/slices/adminSlice";
...
export const UserManagmentTable = () => {
    ...
    const handleStatus = (index: number) =>{
        // if(users){
        //     const userId: string = users[index]._id
        ...
        {users ? (
            <tbody>
                {users.map((user, index)=>{
                    ...
                    const isStatusActive = Boolean(userObject?.isStatus);
                    return(
                    <tr key={index} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        <th scope="row" className="px-6 py-6 w-2/5 font-medium text-gray-900 whitespace-nowrap dark:text-white" >
                        {userObject?.authId?.email}
                        </th>
                        <td className="px-6 py-6 w-1/5">{userObject?.authId?.username}</td>
                        <td className="px-6 py-6 w-1/5">{userObject?.authId?.phoneNumber}</td>
                        <td className="px-6 py-6 w-1/5">
                        <div className="w-1/5 font-medium text-blue-600 dark:text-blue-500 hover:underline" onClick={() => handleStatus(index)}>
                            {isStatusActive ? 
