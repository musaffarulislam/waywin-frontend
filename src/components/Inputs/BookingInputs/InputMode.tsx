...
const InputMode = ({ onOptionMode, modes, error }: optionProps) => {

  const [selectedCheckboxes, setSelectedCheckboxes] = useState<string>("");

  useEffect(() => {
    if(selectedCheckboxes){
      onOptionMode(selectedCheckboxes);
    }
  }, [selectedCheckboxes, onOptionMode]);
                ...
                <div key={index} className="rounded-xl p-2 px-4 bg-slate-200 dark:bg-red-600 mb-0 sm:mb-6 md:mb-0">
                  <div className="flex items-center ">
                    <input
                      ...
                      checked={selectedCheckboxes.includes(mode)}
